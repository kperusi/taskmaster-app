...
import "./dashboardstyle/dashboardstyle.css";
...
export default function DashboardPage() {
  ...
  const [overdueCompleted, setOverdueCompleted] = useState([]);
  ...
  useEffect(() => {
    ...
    const filterTasksByDueDateAndCompleted = () =>
      overdueTask.filter((task) => task.status !== "completed");
    setOverdueCompleted(filterTasksByDueDateAndCompleted);
  }, [tasks]);

  console.log(overdueCompleted);
    ...
    <main className="dashboard-main">
      <div>
        <div>
          ...
          {/* <section className="today-overdue-task-x" >
            {overdueTask.length !== 0 && (
              <div className="overdue-task-x">
                <h3>Overdue Tasks</h3>
                {overdueCompleted.slice(0, 3).map((task) => (
          ...
          <section className="recent-upcoming-x">
