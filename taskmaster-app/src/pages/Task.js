...
export default function Task() {
  ...
  const tasks=useSelector((state)=>state.tasks.tasks)||JSON.parse(localStorage.getItem("tasks")) 
  console.log(tasks)
    ...
    <main className="task-main-container">
        ...
        {todoTask.map((task) => (
          ...
          >
            ...
            <div style={{ marginTop: "15px" }}>
              <p>{`0 of ${task.subtasks.length} Completed`}</p>
              {/* <p>{task._id}</p> */}
            </div>
          </div>
        ))}
      ...
      <section>
        ...
        {taskInProgress.map((task) => (
          <div key={task._id} className="in-progress-container todo-container" onClick={() => handleNavigate(`${task._id}`)}>
            <h3>{task.title}</h3>
            <p>{task.description}</p>
            <div className="row-1">
              <p className={`${task.priority}`}>{task.priority}</p>
            ...
            <div style={{ marginTop: "15px" }}>
              <p>{`${task.subtasks.filter((s)=>s.completed===true).length} of ${task.subtasks.length} Completed`}</p>
            </div>
          </div>
        ))}
      </section>
