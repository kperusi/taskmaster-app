...
export default function Task() {
  ...
  const handleItemSelection = (item) => {
    if (item === "todo") {
      setSelectedItem({ todo: "todo-selected", doing: "", done: "" });
    }
    if (item === "doing") {
      setSelectedItem({ todo: "", doing: "doing-selected", done: "" });
    }
    if (item === "done") {
      setSelectedItem({ todo: "", doing: "", done: "done-selected" });
    }
  };
  ...
  return (
    <main className="task-main-container">
      ...
      <div className="task-mobi-view">
        <nav className="task-mobi-nav">
          <div
            className={`task-mobi-todo-x  ${selectedItem.todo}`}
            onClick={() => handleItemSelection("todo")}
          >
            <div className="circle-title-x">
              <span className="todo-circle circle"></span>
              <h1>{`TODO (${todoTask.length})`}</h1>
          ...
          <div
            className={`task-mobi-doing-x ${selectedItem.doing} `}
            onClick={() => handleItemSelection("doing")}
          >
            <div className="circle-title-x">
              <span className="in-progress-circle circle"></span>
              <h1>{`DOING (${taskInProgress.length})`}</h1>
          ...
          <div
            className={`task-mobi-done-x ${selectedItem.done} `}
            onClick={() => handleItemSelection("done")}
          >
            <div className="circle-title-x">
              <span className="completed-circle circle"></span>
              <h1>{`DONE (${completedTasks.length})`}</h1>
